...
It might be helpful to understand the following: (a) the point cloud is first projected to a plane P estimated via PCA; (b) the projected cloud outlines a plane region D, which is estimated with the aid of a kernel density estimator; (c) thin-plate spline is used to lift D to a smooth surface in 3D space. More details may be found in the paper.-------------------------------------------User Instructions
1. Unzip the downloaded `mesh_regularization.zip’ file into a folder, say, `myfolder’2. Your input data should be a point cloud in .OFF format, say `pointcloud.OFF’. Note that it is fine for the .OFF file to contain additional information (e.g., mesh structure). If your data is in some other format (such as .PLY or .OBJ), you can use the free software Meshlab (http://meshlab.sourceforge.net/) to convert the file into .OFF format.3. Save the `pointcloud.OFF’ file to `myfolder’.4. Open Matlab and navigate to `myfolder’. Within Matlab, execute the command      		>>mesh_regularizationYou will be prompted to 'give the name of original .OFF file’. Please type in the name of the original file between quotes without the file extension. For example, ‘pointcloud’
